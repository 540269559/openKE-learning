...
class TransH(Model):
	...
	def __init__(self, ent_tot, rel_tot, dim = 100, p_norm = 1, norm_flag = True, margin = None, epsilon = None):
        ...
        # 是否是手动定义范围还是自动交给nn.init.xavier_uniform_定义嵌入向量的数值范围
    ...
    # 这个函数的作用是对给定的嵌入向量 e 进行投影操作，
    # 以使其与给定的正则化向量 norm 平行。让我逐步解释这个函数：
	def _transfer(self, e, norm):
        # L2正则化平面向量
		norm = F.normalize(norm, p = 2, dim = -1)
        # 这部分代码检查 e 的第一个维度是否与 norm 的第一个维度相同。
        # 如果不同，说明需要对 e 进行批次处理，以适应 norm 的形状。
		if e.shape[0] != norm.shape[0]:
			e = e.view(-1, norm.shape[0], e.shape[-1])
			norm = norm.view(-1, norm.shape[0], norm.shape[-1])
            # 这一行代码执行向量投影操作。
            # 首先，计算 e 和 norm 的逐元素乘积，然后在最后一个维度上求和，得到一个形状为 (batch_size, embedding_dim) 的张量。
            # 接着，将其扩展为 (batch_size, 1, embedding_dim) 的张量，以便进行广播。最后，将 e 减去这个投影结果，从而使 e 与 norm 平行。
